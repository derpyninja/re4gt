...
dfs$EF44_bin10 <- cut_width(dfs$EF44, width=10, boundary=0)
...
vars_summary <- c("EF952", "EF46", "EF44", "EF44_bin10", "EF517", "EF540", "EF442", "EF114UG4", "EF114UG3", "EF137UG1")
...
# versions:
occ_list_versions <- c(
  "gbn_class_short_list", "gbn_class_short_list_tobi"
  # "full_list",
  # "long_list_green", "long_list_brown",
  # "short_list_green", "short_list_brown",
  # "short_list_brown_tobi"
  )
...
for (occ_list_version in occ_list_versions) {
  ...
  occupation_metadata <-
    # read
    read_excel(
    path = file.path(metadatenpfad, "occ_metadata_en_kldb2010_final_merged.xlsx"),
  ...
  # -----------------------------------------------------------------------------
  # --> AB HIER BRAUCHE ICH ifelse UNTERSCHEIDUNG FÜR GBN KLASSIFIKATIONS-SHEETS
  # -----------------------------------------------------------------------------
  input_is_clsf <- str_detect(occ_list_version, "class")
  if(!(input_is_clsf)){
    # -----------------------------------------------------------------------------
    ...
    write_csv(EF952_by_EF114_abs, file=file.path(outputpfad, occ_list_version, "EF952_by_EF114_abs.csv"))
    ...
    write_csv(EF952_by_EF189_rel, file=file.path(outputpfad, occ_list_version, "EF952_by_EF189_rel.csv"))
    ...
    write_csv(EF952_by_EF189_abs, file=file.path(outputpfad, occ_list_version, "EF952_by_EF189_abs.csv"))
    ...
    write_csv(EF952_by_EF564_rel, file=file.path(outputpfad, occ_list_version, "EF952_by_EF564_rel.csv"))
    ...
    write_csv(EF952_by_EF564_abs, file=file.path(outputpfad, occ_list_version, "EF952_by_EF564_abs.csv"))
    ...
    write_csv(EF952_by_EF137_rel, file=file.path(outputpfad, occ_list_version, "EF952_by_EF137_rel.csv"))
    ...
    write_csv(EF952_by_EF137_abs, file=file.path(outputpfad, occ_list_version, "EF952_by_EF137_abs.csv"))
    ...
    write_csv(EF952_by_EF137UG1_rel, file=file.path(outputpfad, occ_list_version, "EF952_by_EF137UG1_rel.csv"))
    ...
    write_csv(EF952_by_EF137UG1_abs, file=file.path(outputpfad, occ_list_version, "EF952_by_EF137UG1_abs.csv"))
    ...
    # with sex
    cor.test(x = as.integer(dfm$EF46), y = dfm$share_green_esco)
    ...
    # with income
    cor.test(x = as.integer(dfm$EF436), y = dfm$share_green_esco)
    cor.test(x = as.integer(dfm$EF442), y = dfm$share_green_esco)
    ...
    cor.test(x = as.integer(dfm$EF517), y = dfm$share_green_esco)
    cor.test(x = as.integer(dfm$EF540), y = dfm$share_green_esco)
    ...
    print("analysing categorical classifications")
    
    # params
    vars_summary_class <- c("EF952", "EF46", "EF44", "EF44_bin10", "EF517", "EF540", "EF442", "EF114UG4", "EF137UG1")
    fill_colors <- c("green" = "darkgreen", "brown" = "brown", "neutral" = "darkgrey")
    category_versions <- c("gbn_category", "gbn_category_filled")
    
    for (category_version in category_versions){
      # summary tables incl. tests for significant differences btw groups
      # -------------------------------------------------------------------------
      
      # create dummies for simulating 3 categories in KDFV data set
      dfm[[category_version]] <- replace(dfm[[category_version]], is.na(dfm[[category_version]]), sample(c("green", "brown"), sum(is.na(dfm[[category_version]])), replace = TRUE))
      ...
      ggplot(dfm, aes(x = EF442, y=EF952, fill = .data[[category_version]])) +
        geom_col(position = "dodge") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
        labs(x="Income [Euro/month] (EF442)", y="Population [-] (EF952)") +
        scale_fill_manual(values = fill_colors)
...
# -----------------------------------------------------------------------------
# 2.4) Distribution of KldB 5-digit occupations within broader ISCO groups
# -----------------------------------------------------------------------------

# select variables
vars_occ_dist <- c("n_obs", "EF952", "EF114", "EF541", "EF541UG1", "EF541UG2")
df_occ_dist <- dfm %>%
  select(all_of(vars_occ_dist))

# obtain distribution within ISCO 4/3/2-digit groups
df_occ_dist_4d <- df_occ_dist %>% group_by(EF541, EF114) %>% summarise(n_obs=sum(n_obs), EF952=sum(EF952))
df_occ_dist_3d <- df_occ_dist %>% group_by(EF541UG1, EF114) %>% summarise(n_obs=sum(n_obs), EF952=sum(EF952))
df_occ_dist_2d <- df_occ_dist %>% group_by(EF541UG2, EF114) %>% summarise(n_obs=sum(n_obs), EF952=sum(EF952))

# save
write_csv(df_occ_dist_4d, file=file.path(outputpfad, "EF114_within_EF541_nobs.csv"))
write_csv(df_occ_dist_3d, file=file.path(outputpfad, "EF114_within_EF541UG1_nobs.csv"))
write_csv(df_occ_dist_2d, file=file.path(outputpfad, "EF114_within_EF541UG2_nobs.csv"))
